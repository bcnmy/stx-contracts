...
abstract contract ERC7739Validator {
        ...
        returns (bytes4)
    {
        ...
        // sig malleability prevention is not needed here as we now act in the
        // sig type context agnostic environment. it can be non ecdsa so this check
        // is not applied here
        /*
        bytes32 s;
        assembly {
        ...
        */
        ...
        bool success = _erc1271IsValidSignatureViaSafeCaller(sender, account, hash, signature)
            || _erc1271IsValidSignatureViaNestedEIP712(account, hash, signature)
            || _erc1271IsValidSignatureViaRPC(account, hash, signature);
