...
abstract contract NexusTestBase is BaseTest, EventsAndErrors {
    ...
    function buildInitCode(address ownerAddress, address validator) internal view returns (bytes memory initCode) {
        ...
        BootstrapConfig[] memory validators = NexusBootstrapLib.createArrayConfig(validator, moduleInitData);
        BootstrapConfig memory hook = NexusBootstrapLib.createSingleConfig(address(0), "");
