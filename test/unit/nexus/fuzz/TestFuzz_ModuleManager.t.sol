...
contract TestFuzz_ModuleManager is TestModuleManagement_Base {
    ...
    function testFuzz_InstallModule_WithInvalidParameters(uint256 randomTypeId, address randomAddress) public {
        ...
        bytes memory callData =
            abi.encodeWithSelector(IModuleManager.installModule.selector, randomTypeId, randomAddress, "");
        ...
        assertFalse(
            ...
            "Module installation should fail with invalid parameters"
        );
    }
    ...
    function testFuzz_InstallModule_CorrectType(uint256 moduleTypeId, address moduleAddress, bytes4 funcSig) public {
        ...
        bytes memory callData =
            abi.encodeWithSelector(IModuleManager.installModule.selector, moduleTypeId, moduleAddress, initData);
        Execution[] memory executions = new Execution[](1);
        executions[0] = Execution(address(BOB_ACCOUNT), 0, callData);
        PackedUserOperation[] memory userOps =
            buildAndSignPackedUserOp(BOB, BOB_ACCOUNT, EXECTYPE_DEFAULT, executions, address(VALIDATOR_MODULE), 0);
        ...
        if (!IModule(moduleAddress).isModuleType(moduleTypeId)) {
            ...
            assertTrue(
                ...
                "Module should be correctly installed"
            );
        }
    }
        ...
        if (!IModule(moduleAddress).isModuleType(moduleTypeId)) {
            ...
            if (moduleTypeId == MODULE_TYPE_FALLBACK) {
                expectedRevertReason =
                    abi.encodeWithSignature("FallbackAlreadyInstalledForSelector(bytes4)", bytes4(funcSig));
            ...
            emit UserOperationRevertReason(
                userOpHash, address(BOB_ACCOUNT), userOpsSecondAttempt[0].nonce, expectedRevertReason
            );
    ...
    function testFuzz_UninstallModule(uint256 moduleTypeId, address moduleAddress, bytes4 funcSig) public {
        ...
        if (moduleTypeId == MODULE_TYPE_VALIDATOR) {
            ...
            callData = abi.encodeWithSelector(
                IModuleManager.uninstallModule.selector, moduleTypeId, moduleAddress, abi.encode(prev, "")
            );
            ...
            callData =
                abi.encodeWithSelector(IModuleManager.uninstallModule.selector, moduleTypeId, moduleAddress, initData);
        }
    ...
    function testFuzz_UninstallPreviousModule(uint256 moduleTypeId, address moduleAddress, bytes4 funcSig) public {
        ...
        if (moduleTypeId == MODULE_TYPE_VALIDATOR) {
            ...
            address prev = SentinelListHelper.findPrevious(array, remove);
            callData = abi.encodeWithSelector(
                IModuleManager.uninstallModule.selector, moduleTypeId, address(VALIDATOR_MODULE), abi.encode(prev, "")
            );
        } else if (moduleTypeId == MODULE_TYPE_EXECUTOR) {
            // Prepare the uninstallation calldata for Executor
            (address[] memory array,) = BOB_ACCOUNT.getExecutorsPaginated(address(0x1), 100);
            ...
            address prev = SentinelListHelper.findPrevious(array, remove);
            callData = abi.encodeWithSelector(
                IModuleManager.uninstallModule.selector, moduleTypeId, address(EXECUTOR_MODULE), abi.encode(prev, "")
            );
        } else {
            // Prepare the uninstallation calldata for other module types
            callData =
        ...
        if (moduleTypeId == MODULE_TYPE_VALIDATOR) {
            assertFalse(
                BOB_ACCOUNT.isModuleInstalled(moduleTypeId, address(VALIDATOR_MODULE), initData),
                "Module should be uninstalled"
            );
        } else if (moduleTypeId == MODULE_TYPE_EXECUTOR) {
            assertFalse(
                BOB_ACCOUNT.isModuleInstalled(moduleTypeId, address(EXECUTOR_MODULE), initData),
                "Module should be uninstalled"
            );
        } else {
            assertFalse(
    ...
    /// @param moduleAddress The address of the module
    /// @param funcSig The function signature for the module
    function testFuzz_UninstallWithMismatchedModuleType(
        ...
        public
    {
        ...
        if (!IModule(moduleAddress).isModuleType(moduleTypeId)) {
            bytes memory expectedRevertReason =
                abi.encodeWithSelector(ModuleNotInstalled.selector, moduleTypeId, moduleAddress);
            bytes32 userOpHash = ENTRYPOINT.getUserOpHash(userOps[0]);
            vm.expectEmit(true, true, true, true);
            emit UserOperationRevertReason(
    ...
    function testFuzz_UninstallNonInstalledModule(uint256 moduleTypeId, address moduleAddress, bytes4 funcSig) public {
        ...
        bytes memory expectedRevertReason =
            abi.encodeWithSignature("ModuleNotInstalled(uint256,address)", moduleTypeId, moduleAddress);
        bytes32 userOpHash = ENTRYPOINT.getUserOpHash(userOps[0]);
        vm.expectEmit(true, true, true, true);
        emit UserOperationRevertReason(
            userOpHash, // userOpHash
        ...
        assertFalse(
            BOB_ACCOUNT.isModuleInstalled(moduleTypeId, moduleAddress, initData), "Module should remain uninstalled"
        );
    }
