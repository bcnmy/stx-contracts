...
import { MockTransferer } from "../../../../mock/MockTransferer.sol";

contract TestEIP7702 is NexusTestBase {
    ...
    function _getInitData() internal view returns (bytes memory) {
        ...
        return abi.encode(
            ...
            abi.encodeCall(BOOTSTRAPPER.initNexus, (validators, executors, hook, fallbacks, preValidationHooks))
        );
    }
    ...
    function test_execSingleFromExecutor() public {
        ...
        bytes[] memory ret = mockExecutor.executeViaAccount(
            INexus(address(account)), address(target), 0, abi.encodePacked(MockTarget.setValue.selector, uint256(1338))
        );
