...
contract TestAccountExecution_ExecuteFromExecutor is TestAccountExecution_Base {
    ...
    function test_TryExecuteBatch_SingleFailure() public {
        ...
        emit TryExecuteUnsuccessful(
            executions[1].callData, abi.encodeWithSignature("Error(string)", "Counter: Revert operation")
        );
    ...
    function test_TryExecuteBatch_MultipleFailures() public {
        ...
        vm.expectEmit(true, true, true, true);
        emit TryExecuteUnsuccessful(
            executions[0].callData, abi.encodeWithSignature("Error(string)", "Counter: Revert operation")
        );
        vm.expectEmit(true, true, true, true);
        emit TryExecuteUnsuccessful(
            executions[1].callData, abi.encodeWithSignature("Error(string)", "Counter: Revert operation")
